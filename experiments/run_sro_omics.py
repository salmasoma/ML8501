...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory where convergence and comparison plots will be saved (default: ./figures).",
    )
    parser.add_argument(
        "--no-figures",
        action="store_true",
        help="Disable figure generation and table exports.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def generate_visualisations(
    ...
    try:
        ...
        print(f"Warning: skipping visualisations because matplotlib/seaborn are unavailable ({exc}).")
        return

    sns.set_theme(style="whitegrid")
    plot_metric_bars(results, figure_dir, plt, sns)
    plot_convergence(histories, figure_dir, plt)


def plot_metric_bars(
    results: pd.DataFrame, figure_dir: Path, plt, sns
) -> None:
    metrics = ["mae", "rmse", "r2"]
...
def plot_convergence(histories: List[Dict[str, object]], figure_dir: Path, plt) -> None:
    if not histories:
        return
